...
if (empty($count_products)) {
    ...
    $uniq_arr_catalog = array_unique($arSectionCatalog); //301,302,308
    ...
    if ($count_catalogs > 1) {
        $numcataogs = 0;
         if (in_array("$CatSout", $uniq_arr_catalog)){
            $numcataogs = $numcataogs+1;
        }
        if (in_array("$CatPk", $uniq_arr_catalog) || in_array("$CatRad", $uniq_arr_catalog)){
            $numcataogs = $numcataogs+1;
        }
        if (in_array("$CatOpr", $uniq_arr_catalog)){
            $numcataogs = $numcataogs+1;
        }
        if (in_array("$CatLearn", $uniq_arr_catalog) || in_array("$CatLearn1", $uniq_arr_catalog) || in_array("$CatLearn2", $uniq_arr_catalog) ){
            $numcataogs = $numcataogs+1;
        }
        if (in_array("$CatAudit", $uniq_arr_catalog)){
            $numcataogs = $numcataogs+1;
        }
        if (in_array("$CatAutsors", $uniq_arr_catalog)){
            $numcataogs = $numcataogs+1;
        }

        if ($numcataogs > 0) {
            $rootActivity->SetVariable("CatalogNum",$numcataogs);
        }
    }
