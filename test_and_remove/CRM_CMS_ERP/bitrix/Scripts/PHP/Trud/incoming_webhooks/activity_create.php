...
$activity_assigned = $arActivty['RESPONSIBLE_ID'];
    $activity_missed = (isset($arActivty['SETTINGS']['MISSED_CALL']) 
                                ? $arActivty['SETTINGS']['MISSED_CALL'] : '');
           ...
           // writeToLog($assigned_deal, "activity:$activity_assigned");
            if ($activity_completed == 'N') {
                if ($activity_missed) { // if bool true
                    $arDealId = [];
                    $resBindings = activityGetBindings('crm.activity.binding.list', $activity_id); //get all bindings activity
