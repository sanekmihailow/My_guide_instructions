...
ChangeConfigInc(){
    c_inc="$(echo $(cat ${path_def}/etalon.inc.php > ${path_cur}/${vtname}/crm4team/config.inc.php))"
    genkey="$(echo $(head /dev/urandom | tr -dc a-z0-9 | head -c 32 ; echo ''))"
    changelist="25s/DBusername/${vtname}/ 26s/DBpassword/${vtpass}/ 27s/DBname/${vtname}/ 41s/DomainName/${vtname}.crm4team.com/ 45s/VtigerName/${vtname}/ 79s/RandomText/${genkey}/"

    for i in $changelist; do
        sed -i "${i}" "$path_cur"/"$vtname"/crm4team/config.inc.php
    done
...
CreateConf(){
    ...
    if [[ "$apache_check" == 'Syntax OK' ]]; then
            if [[ $(grep "$vtname" /etc/hosts) ]]; then
                ...
                echo "127.0.0.1 ${vtname}.crm4team.com" >> /etc/hosts
            fi

            certbot certonly --apache --non-interactive --agree-tos -m noreply-crm4team-vtiger@yandex.ru -d "$vtname".crm4team.com 2> /dev/null
            cp "$path_def"/etalon-redirect-https.conf "$path_apache"/"$vtname".conf
            rm "$path_apache"/"$vtname"-le-ssl.conf 2> /dev/null
            sed -i "41s/http\:/https\:/" "$path_cur"/"$vtname"/crm4team/config.inc.php
            ...
            if [[ "$apache_check2" == 'Syntax OK' ]]; then
                ...
                #mail
                #M mail -s 'Error when create new user' noreply-crm4team-vtiger@yandex.ru < "$path_temp"/error_cr_conf.txt
                exit 1
            fi
    fi
}
