...
```nginx
    sudo apt install htop mc links2 inxi lm-sensors lsscsi rsync p7zip tar unzip rar vim vim-gtk xorg openbox iperf speedtest-cli bash-completion mytop ftp-upload grc smartmontools hddtemp keychain xstow xclip git nmap tcpdump tshark build-essential netcat telnet mtr hexcure
```
#### GUI
```nginx
