...
```nginx
    sudo apt install htop mc links2 inxi lm-sensors lsscsi rsync p7zip tar unzip rar vim vim-gtk xorg openbox iperf speedtest-cli bash-completion mytop ftp-upload grc smartmontools hddtemp keychain xstow xclip git nmap tcpdump tshark build-essential netcat telnet mtr hexcurse
```
#### GUI
```nginx
...
#### CLI
```nginx
sudo pacman -S yaourt base-devel fakeroot jshon expac git wget xorg xorg-server exfat-utils p7zip p7zip-plugins unrar tar rsync htop mc inxi lm_sensors lsscsi openssh namcap python-pip gvim iperf speedtest-cli iptables bash-completion innotop mytop sysstat smartmontools hddtemp keychain ssh-ident.git freerdp libvncserver sngrep mtr hexedit
```
