...
#### CLI
```nginx
sudo pacman -S yaourt base-devel fakeroot jshon expac git wget xorg xorg-server exfat-utils p7zip p7zip-plugins unrar tar rsync htop mc inxi lm_sensors lsscsi openssh namcap python-pip gvim iperf speedtest-cli iptables bash-completion innotop mytop sysstat smartmontools hddtemp keychain ssh-ident.git
```
...
##### src
    ...
    then extract src.tar.gz --.> cd in folder, after makepkg (without -s)
