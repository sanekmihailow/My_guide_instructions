...
#### CLI
```nginx
    sudo apt install htop mc links2 inxi sensors lsscsi rsync p7zip tar unzip rar vim vim-gtk xorg openbox iperf speedtest-cli bash-completion innotop mytop ftp-upload grc smartmontools vtop hddtemp keychain xstow xclip git nmap tcpdump sngrep tshark build-essential
```
#### GUI
```nginx
