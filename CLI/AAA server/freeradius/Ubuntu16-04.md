...
rad_recv: Access-Accept packet from host 127.0.0.1 port 1812, id=45, length=20
```
### 5) push freeradius sql schema
```nginx
            mysql -u root -p database < /usr/local/stow/freeradius-3/etc/raddb/mods-config/sql/main/mysql/schema.sql
```
### 4) edit sql.conf           
```nginx

            sudo vim /usr/local/stow/freeradius-3/etc/raddb/mods-available/sql
```
```bash
sql {
driver = "rlm_sql_mysql"
...
### 5) edit site conf
```nginx
            sudo vim /usr/local/stow/freeradius-3/etc/raddb/sites-enabled/default
```
change "-sql" on "sql" in sections
```bash
authorize {
sql
}
